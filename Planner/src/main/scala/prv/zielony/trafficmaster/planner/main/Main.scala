package prv.zielony.trafficmaster.planner.main

import akka.actor.ActorSystem
import prv.zielony.trafficmaster.planner.utils.ClasspathProperties
...
object Main extends SimpleRoutingApp {
  ...
  def main(args:Array[String]):Unit = {
    ...
    this.startServer(deploymentProperties.getProperty("microservices.planner.server.host"),
      Integer.parseInt(deploymentProperties.getProperty("microservices.planner.server.port"))) {
      path("version") {
        complete("1.0")
      }
    }
